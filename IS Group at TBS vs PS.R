...
subThemeOrder <- demoMeans.df %>%
  filter(BYCOND == "M_Q103A = 060" & variable == "NEGATIVE") %>%
  arrange(value)
...
# Select demographic groups to plot: IS group, SCMA sector, and comms community plus PS and TBS summary columns for comparison
demoMeans.df <- subset(demoMeans.df, BYCOND %in% c("M_Q103A = 060","TBS","PS","LEVEL3ID = 028","M_Q104 = 5") | 
                         DemQ_E %in% c("IS")) 
...
expl_E <- "Each cell of this chart displays the proportion of responses for a particular question theme and demographic category. The bars over each column represent the average value for this question theme for TBS. Question themes are sorted from least negative to most negative (the red column) for the IS group at TBS."
expl_E <- paste0(strwrap(expl_E, 100), sep="", collapse="\n")
ttl_E <- "PSES@TBS 2017 - IS Group"
...
PSdemoMeans.df <- filter(demoMeans.df, LEVEL1ID == "0" | BYCOND == "TBS" | BYCOND == "LEVEL3ID = 028")
...
PSoverall <- PSdemoMeans.df %>%
  filter(BYCOND %in% c("PS","TBS","LEVEL3ID = 028")) %>%
  arrange(QSubTheme_E, variable, OrderKey) %>%
  group_by(QSubTheme_E, variable) %>%
  mutate(value = value[1])

PSdetail <- filter(PSdemoMeans.df, !(BYCOND %in% c("PS","TBS","LEVEL3ID = 028")))
...
plotIS <- function(language, wdth = 10, hght = 8, textSize = 9) {
  ...
  ggplot(data=TBSdemoMeans.df, aes(x=variable_lang, y=value, fill = variable_lang)) +
    ...
    facet_grid(QSubTheme_lang ~ DESCRIP_lang, switch = "y", scales = "free_y",
               labeller = labeller(DESCRIP_lang = label_wrap_gen(15), QSubTheme_lang = label_wrap_gen(25)))  +
