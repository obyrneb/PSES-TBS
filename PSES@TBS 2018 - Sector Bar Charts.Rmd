...
slp.grb <- plot_grid(slp_ttl,slp,nrow=2,rel_heights = c(1,12))
slp.grb
```

```{r Sector Card - Shifts, echo=FALSE, fig.height=8, fig.width=10}
...
best <- ggplot(data = bestData, x = abbr_E, group = abbr_E) +
  ...
  geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
            size = 2, colour = "grey30", fontface = "plain", hjust = 1.3) +
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 2, colour = "grey30", fontface = "bold", hjust = -0.3)+
...
worst <- ggplot(data = worstData, x = abbr_E, group = abbr_E) +
  ...
  #geom_point(aes(x = abbr_E, y = `2018`, alpha = abbr_E), size = 8, shape = 16, colour = "red") +
  geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
            size = 2, colour = "grey30", fontface = "plain", hjust = -0.3) +
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 2, colour = "grey30", fontface = "bold", hjust = 1.3)+
  geom_text(aes(label = paste0("(",delta,")"), x = abbr_E, y = `2018`, colour = delta),
            size = 2, fontface = "bold", hjust = 2.2)+
  coord_flip() +
  facet_grid(substr(TITLE_E,10,200)~.,switch = "y", labeller = label_wrap_gen(50)) +
  #scale_alpha_manual(values = c(1,.5)) +
  scale_colour_gradient2(high = "#0571b0", mid = "#f7f7f7", low = "#ca0020") +
...
sectorHarDis <- question100s %>%
  filter(unitcode %in% c(thisSector, "TBS") & SURVEYR == 2018 & QUESTION %in% c("Q48","Q55")) %>%
  mutate(abbr_E = ifelse(unitcode == thisSector, thisAbbr_E, unitcode)) %>%
...
har <- ggplot(filter(sectorHarDis, QUESTION == "Q55"), x = abbr_E, group = abbr_E) +
  geom_segment(aes(x = abbr_E, xend = abbr_E, y = `2017`, yend = `2018`, colour = delta),
               size = 1, linejoin = "mitre", arrow = arrow(length = unit(0.2, "cm"))) +
  #geom_point(aes(x = abbr_E, y = `2017`, alpha = abbr_E), size = 8, shape = 16, colour = "blue") +
  #geom_point(aes(x = abbr_E, y = `2018`, alpha = abbr_E), size = 8, shape = 16, colour = "red") +
  geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
            size = 2, colour = "grey30", fontface = "plain", vjust = 1.5) +
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 2, colour = "grey30", fontface = "bold", vjust = 1.5)+
  geom_text(aes(label = paste0("(",delta,")"), x = abbr_E, y = (`2017` + delta/2), colour = delta),
            size = 2, fontface = "bold", vjust = 1.5)+
  coord_flip() +
  facet_grid(abbr_E~.,switch = "y", labeller = label_wrap_gen(50)) +
  #scale_alpha_manual(values = c(1,.5)) +
  scale_colour_gradient2(high = "#0571b0", mid = "#f7f7f7", low = "#ca0020") +
  #scale_y_continuous(limits = c(25,100), breaks = c(25,50,75,100)) +
  deltaTheme
...
har <- textGrob("Harassment Plot")
dis <- textGrob("Discrimination Plot")

hd.grb <- plot_grid(har,dis)  
