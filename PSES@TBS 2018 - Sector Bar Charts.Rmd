...
sectorDeltas <- question100s %>%
  filter(unitcode %in% c(thisSector)) %>%
  mutate(abbr_E = ifelse(unitcode == thisSector, thisAbbr_E, unitcode)) %>%
...
best <- ggplot(data = inner_join(sectorDeltas, best10deltas, by = "QUESTION"),
  ...
  coord_flip() +
  facet_grid(substr(TITLE_E,10,200)~.,switch = "y", labeller = label_wrap_gen(80)) +
...
shifts.grb <- plot_grid(best_ttl,best,worst_ttl,worst, nrow = 4, rel_heights = c(1,12,1,12))
