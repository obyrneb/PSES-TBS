...
demoMeans.df <- aggregate(data = ss.df, cbind(ANSCOUNT,SCORE100,NEGATIVE,NEUTRAL,POSITIVE) ~ LEVEL1ID + QSection + QUESTION + DemoQ + BYCOND + DESCRIP_E + DESCRIP_F + SURVEYR, mean)
demoMeans.df <- melt(demoMeans.df, id = 1:10)
...
file_E <- "PSES 2017 @ TBS - Question Categories by Demographic.pdf"
...
file_F <- "SAFF 2017 @ SCT - Catégories de question par secteur.pdf"
...
#EN plot
plotDemoSM <- function(language, output = "none") {
...
ggplot(data=demoMeans.df, aes(x=SURVEYR,y=value, fill = variable_lang)) +
...
ggsave(file_lang, height = 8, width = 10)
