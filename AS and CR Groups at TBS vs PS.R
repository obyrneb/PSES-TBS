...
# Select demographic groups to plot: AS and CR groups, plus PS and TBS summary columns for comparison
...
demoMeans.df$variable_E <- mapvalues(demoMeans.df$variable, 
                                     ...
                                     c("Negative","Neutral","Positive"))
PNN_E.lbls <- c("Negative" = "Naegative", "Neutral" = "Neutral", "Positive" = "Positive", 
...
PSdemoMeans.df <- filter(demoMeans.df, LEVEL1ID == "0")
...
plotAS_CR <- function(language, wdth = 10, hght = 8) {
  ...
  ggplot(data=TBSdemoMeans.df, aes(x=variable_lang, y=value, fill = variable_lang)) +
    ...
    geom_errorbar(data = PSdemoMeans.df, aes(ymax=value, ymin=value, linetype = PSmean_lang), colour = "grey20") +
