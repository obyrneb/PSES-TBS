...
report_card <- function(thisSector, question100s, score100s, customName = NULL, customAbbr = NULL) {
  ...
  best.plt <- ggplot(data = bestData, x = abbr_E, group = abbr_E) +
    ...
    geom_point(aes(x = abbr_E, y = (`2017`+delta/2), colour = delta), shape = 62, size = 2) +
    ...
    geom_text(aes(label = paste0("+",delta), x = abbr_E, y = 0, colour = delta),
              size = 3, fontface = "bold", hjust = 0, vjust = 0.5) +
    coord_flip() +
    facet_grid(fct_reorder(paste0(INDICATORENG,": Q",substr(TITLE_E,10,140)),
  ...
  worst.plt <- ggplot(data = worstData, x = abbr_E, group = abbr_E) +
    ...
    geom_hline(aes(yintercept = `2017`), colour = "grey60", linetype = 3) +
    geom_hline(aes(yintercept = `2018`, colour = delta)) +
    geom_point(aes(x = abbr_E, y = (`2017`+delta/2), colour = delta), shape = 60, size = 2) +
    geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
  ...
  harNature.ttl <- textGrob("Nature of Harassment (2018)",
                     gp = gpar(fontsize = 10, fontface = "bold", col = "grey30"))
  disType.ttl <- textGrob("Type of Discrimination (2018)",
                     gp = gpar(fontsize = 10, fontface = "bold", col = "grey30"))
  ...
  top.grb <- textGrob(ttl_E, x = unit(0.01, "npc"), just= "left",
