...
demoMeans.df <- subset(demoMeans.df, DemoQ %in% c("M_Q116","M_Q117","M_Q118","M_Q103A","TBS"))
...
ttl_E <- "PSES@TBS 2017 - Occupational and Employment Equity Groups by Question Theme"
...
ttl_F <- "SAFF@SCT 2017 - Groupes professionnels et visés pas l'équité en emploi par thème de question" 
...
plotDemoByTheme <- function(language, wdth = 10, hght = 8) {
  ...
  ggplot(data=demoMeans.df, aes(x=variable_lang, y=value, fill = variable_lang)) +
    ...
    geom_rect(data = subset(demoMeans.df,DemoQ %in% c("M_Q116","M_Q118")),
    ...
    facet_grid(QSubTheme_lang ~ DESCRIP_lang, switch = "y", scales = "free_y",
               labeller = labeller(DESCRIP_lang = label_wrap_gen(30), QSubTheme_lang = label_wrap_gen()))  +
