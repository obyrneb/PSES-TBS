...
dept <- c(e = filter(sector_qs, unitcode == 26) %>% pull(DESCRIP_E) %>% pluck(1),
          f = filter(sector_qs, unitcode == 26) %>% pull(DESCRIP_F) %>% pluck(1))
...
sector_qs <- sector_qs %>% 
  mutate(highlight = case_when(
    unitcode == 0 ~ PS[[lang]],
    unitcode == 26 ~ dept[[lang]],
    unitcode == 310 ~ this_sector[[lang]],
...
line_plot_e <- ggplot(sector_qs, aes(x = QUESTION, y = SCORE100)) +
  ...
  facet_grid(cols = vars(matches(paste0('INDICATOR_',lang),ignore.case = TRUE)), #rows = vars(DemoQ_E),
