...
```{r Sector Card - slope, echo=FALSE, fig.height=8, fig.width=10}
...
ttl_E <- paste0("PSES@TBS 2018 - ",sectorName)
...
slp <- ggplot(sectorData, aes(x = SURVEYR, y = round(indicator100,0), group = INDICATORENG)) +
  ...
  geom_text_repel(data = sectorData %>% filter(SURVEYR == 2018), 
                  ...
                  nudge_x = 1, 
                  direction = "y") +
  geom_point(colour = "grey95", size = 8, shape = 16) +
  ...
  labs(
    title = "Score 100 comparison between..."
  )
...
ggsave(paste0(ttl_E,".pdf"), height = 8, width = 10)
```
...
sectorDeltas <- question100s %>%
  filter(unitcode %in% c(thisSector,"TBS")) %>%
  mutate(abbr_E = ifelse(unitcode == thisSector, thisAbbr_E, unitcode)) %>%
  select(INDICATORENG,INDICATORFRA,QUESTION,TITLE_E,TITLE_F,unitcode,abbr_E,DESCRIP_E,DESCRIP_F,SURVEYR,SCORE100) %>%
...
best <- ggplot(data = inner_join(sectorDeltas, best10deltas, by = "QUESTION"),
                             x = abbr_E, group = abbr_E) +
  geom_segment(aes(x = abbr_E, xend = abbr_E, y = `2017`, yend = `2018`, colour = INDICATORENG, alpha = abbr_E), 
               size = 1.5, lineend = "round", arrow = arrow(length = unit(0.2, "cm"))) +
  ...
  geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
            size = 4, colour = "grey30", fontface = "bold", hjust = 1.2) +
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 4, colour = "grey30", fontface = "bold", hjust = -0.2) +
  geom_text(aes(label = paste0("+",delta), x = abbr_E, y = (`2017` + delta/2)),
            size = 4, colour = "grey30", fontface = "bold", vjust = 1.1) +
  coord_flip() +
  facet_grid(TITLE_E~.,switch = "y", labeller = label_wrap_gen(45)) +
  ...
  #  Labelling as desired
  labs(
    title = "Top 10 - Improved Questions"
  )

best

worst <- ggplot(data = inner_join(sectorDeltas, worst10deltas, by = "QUESTION"),
                             x = abbr_E, group = abbr_E) +
  geom_segment(aes(x = abbr_E, xend = abbr_E, y = `2017`, yend = `2018`, colour = INDICATORENG, alpha = abbr_E), 
               size = 1.5, lineend = "round", arrow = arrow(length = unit(0.2, "cm"))) +
  #geom_point(aes(x = abbr_E, y = `2017`, alpha = abbr_E), size = 8, shape = 16, colour = "blue") +
  ...
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 4, colour = "grey30", fontface = "bold", hjust = 1.2) +
  geom_text(aes(label = delta, x = abbr_E, y = (`2017` + delta/2)),
            size = 4, colour = "grey30", fontface = "bold", vjust = 1.1) +
  coord_flip() +
  facet_grid(TITLE_E~.,switch = "y", labeller = label_wrap_gen(45)) +
  scale_alpha_manual(values = c(1,.5)) +
  ...
  #  Labelling as desired
  labs(
    title = "Bottom 10 - Worsened Questions"
  )
