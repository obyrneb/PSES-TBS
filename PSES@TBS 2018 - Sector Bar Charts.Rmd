...
slp <- ggplot(sectorData, aes(x = SURVEYR, y = round(indicator100,0), group = INDICATORENG)) +
  ...
  geom_line(data = sectorData %>% filter(unitcode == "TBS"),
            aes(color = INDICATORENG), alpha = 0.67,linetype = 2, size = 1) +
  scale_colour_brewer(palette = "Set2") +
...
slp2 <- ggplot(sectorData, aes(x = SURVEYR, y = round(indicator100,0), group = INDICATORENG)) +
  facet_grid(.~abbr_E) + 
  geom_line(data = sectorData %>% filter(unitcode == thisSector),
            aes(color = INDICATORENG), alpha = 0.7,linetype = 1, size = 1) +
  geom_line(data = sectorData %>% filter(unitcode == "TBS"),
            aes(color = INDICATORENG), alpha = 0.7,linetype = 2, size = 1) +
  scale_colour_brewer(palette = "Set2") +
  geom_text_repel(data = sectorData %>% filter(SURVEYR == 2017), 
...
sectorDeltas <- question100s %>%
  filter(unitcode %in% c(thisSector, "TBS")) %>%
  mutate(abbr_E = ifelse(unitcode == thisSector, thisAbbr_E, unitcode)) %>%
  select(INDICATORENG,INDICATORFRA,QUESTION,TITLE_E,TITLE_F,unitcode,abbr_E,DESCRIP_E,DESCRIP_F,SURVEYR,SCORE100,AGREE) %>%
...
best <- ggplot(data = bestData, x = abbr_E, group = abbr_E) +
  ...
  geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
            size = 3, colour = "grey30", fontface = "plain", hjust = 1.3) +
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 3, colour = "grey30", fontface = "bold", hjust = -0.3)+
...
worst <- ggplot(data = worstData, x = abbr_E, group = abbr_E) +
  ...
  #geom_point(aes(x = abbr_E, y = `2018`, alpha = abbr_E), size = 8, shape = 16, colour = "red") +
  geom_text(aes(label = `2017`, x = abbr_E, y = `2017`),
            size = 3, colour = "grey30", fontface = "plain", hjust = -0.3) +
  geom_text(aes(label = `2018`, x = abbr_E, y = `2018`),
            size = 3, colour = "grey30", fontface = "bold", hjust = 1.3)+
  geom_text(aes(label = paste0("(",delta,")"), x = abbr_E, y = `2018`, colour = delta),
            size = 3, fontface = "bold", hjust = 2.2)+
  coord_flip() +
...
```{r Sector Card - Har & Dis, echo=FALSE, fig.height=8, fig.width=10}
...
har_ttl <- textGrob("Harassment", gp = gpar(fontsize = 10, fontface = "bold", col = "grey30"))
dis_ttl <- textGrob("Discrimination", gp = gpar(fontsize = 10, fontface = "bold", col = "grey30"))
hd_ttl.grb <- plot_grid(har_ttl,dis_ttl)

hdpc.grb <- ggplot(data = filter(sectorHarDis, QUESTION %in% c("Q48","Q55")), aes(x = abbr_E)) +
  geom_text(aes(label = paste0(abbr_E,": ",`2018`,"%"),y=1), colour = "grey30", fontface = "italic", size = 4) +
...
harNatureData <- sectorHarDis %>%
  filter(startsWith(QUESTION,"Q50") & unitcode != "TBS") %>%
  mutate(Qshort_E = word(TITLE_E,3, sep = fixed('.')))
harNature <- ggplot(harNatureData, aes(x=fct_reorder(str_wrap(Qshort_E,30),`2018`), y=`2018`)) +
  labs(
    x="Sectors", 
    y="% answering yes in 2018") +
  geom_col(fill = "#7fc97f") +
  geom_text(nudge_y=3, size=3, colour="grey30", fontface = "bold", aes(label=`2018`)) +
  coord_flip() +
  deltaTheme +
...
disTypeData <- sectorHarDis %>%
  filter(startsWith(QUESTION,"Q57") & unitcode != "TBS") %>%
  mutate(Qshort_E = word(TITLE_E,3, sep = fixed('.')))
disType <- ggplot(disTypeData, aes(x=fct_reorder(str_wrap(Qshort_E,30),`2018`), y=`2018`)) +
  labs(
    x="Sectors", 
    y="% answering yes in 2018") +
...
hr_ttl <- textGrob(paste0("Nature of Harassment at ",thisAbbr_E),
                   gp = gpar(fontsize = 8, col = "grey30"))
dt_ttl <- textGrob(paste0("Type of Discrimination at ",thisAbbr_E),
                   gp = gpar(fontsize = 8, col = "grey30"))
...
plot_ttl <- textGrob(paste0("PSES 2018 Report Card - ",sectorName_E),
                     gp = gpar(fontsize = 14, fontface = "bold", col = "grey30"))

plot.grb <- plot_grid(left.grb,right.grb)
plot.grb <- plot_grid(plot_ttl,plot.grb,nrow=2,rel_heights = c(1,20))
plot.grb
```